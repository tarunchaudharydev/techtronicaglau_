...
export default function GizmoEvents() {
  ...
  const gizmoEvents = events.filter((e) => e.club === "Gizmo");

  const upcomingEvents = gizmoEvents.filter(
    (e) =>
      e.registrationDeadline &&
      getISTDeadline(e.registrationDeadline) > getISTNow()
  );

  const pastEvents = gizmoEvents.filter(
    (e) =>
      !e.registrationDeadline ||
      getISTDeadline(e.registrationDeadline) <= getISTNow()
  );
      ...
      <div className="max-w-6xl mx-auto space-y-20">
        ...
        <header className="text-center space-y-3">
          ...
          <p className="text-slate-700 max-w-2xl mx-auto">
            Upcoming and completed robotics events by Gizmo.
          </p>
        </header>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {pastEvents.map((event) => (
              <Link
                key={event.id}
                href={`/clubs/gizmo/events/${event.id}`}
