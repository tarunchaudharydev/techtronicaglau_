...
export function GizmoHeroSection({ aboutGizmo }: Props) {
  return (
        ...
        <div className="space-y-5 text-center lg:text-left flex flex-col items-center lg:items-start animate-fadeInUp">
          ...
          <div className="space-y-2">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight">
              <span className="block text-slate-900">{aboutGizmo.title}</span>
          ...
          {/* Description (ARRAY → mapped correctly) */}
          <div className="space-y-3 max-w-xl mt-3 mb-2">
            {aboutGizmo.description.map((text, i) => (
              <p
                key={i}
                className="text-sm sm:text-base text-slate-800 text-center lg:text-left"
              >
                {text}
              </p>
            ))}
          ...
          <ul className="text-xs sm:text-sm text-slate-700 space-y-1.5 max-w-xl text-left">
            ...
            <li>
              • A friendly, experimental space to ask doubts, test ideas, and
              learn by doing.
            </li>
          </ul>
